...
//					if (cos_angle > 0.0f)
					{
						...
						else
						{
							...
							{
								register SINGLE cone_atten = (cos_angle - l->cos_penumbra) * l->one_over_delta_cos;
										   ...
										   LightRGB * back_rgb)
{
	...
	{
		register ManagedLight ** light_ptr = active_light_map;
		for (l = 0; l < max_lights; l++, light_ptr++)
		{
			register ManagedLight * light = *light_ptr;
			if (light)
			{
