...
const char *CLSID_DcDtBtMaterial="DcDtBt";

//

struct	DcDtBtMaterial : public IMaterial,
						public IMaterialProperties
{
	BEGIN_DACOM_MAP_INBOUND(DcDtBtMaterial)
...
DECLARE_MATERIAL( DcDtBtMaterial, IS_SIMPLE );

//

DcDtBtMaterial::DcDtBtMaterial(void)
{
	init( NULL );
}
...
GENRESULT DcDtBtMaterial::init( DACOMDESC *desc )
{
...
GENRESULT DcDtBtMaterial::cleanup( void ) 
{
...
GENRESULT DcDtBtMaterial::initialize( IDAComponent *system_container ) 
{
	cleanup();
	...
	if( FAILED( system_container->QueryInterface( IID_ITextureLibrary, (void**)&texture_library ) ) ) {
		GENERAL_TRACE_1( "DcDtBtMaterial: intitialize: this material requires IID_ITextureLibrary\n" );
		return GR_GENERIC;
	}
	...
	if( FAILED( system_container->QueryInterface( IID_IRenderPipeline, (void**)&render_pipeline ) ) ) {
		GENERAL_TRACE_1( "DcDtBtMaterial: intitialize: this material requires IID_IRenderPipeline\n" );
		return GR_GENERIC;
	}
	...
	if( FAILED( system_container->QueryInterface( IID_IVertexBufferManager, (void**)&vbuffer_manager ) ) ) {
		GENERAL_TRACE_1( "DcDtBtMaterial: intitialize: this material requires IID_IVertexBufferManager\n" );
		return GR_GENERIC;
	}
...
GENRESULT DcDtBtMaterial::clone( IMaterial **out_material ) 
{
	IMaterial *material;
	DcDtBtMaterial *new_material;
	DACOMDESC desc;

	desc.interface_name = CLSID_DcDtBtMaterial;
	...
	// under CLSID_DcDtBtMaterial
	//
	new_material = (DcDtBtMaterial*)material;
...
GENRESULT DcDtBtMaterial::load_from_filesystem( IFileSystem *IFS ) 
{
	Vector color;
...
GENRESULT DcDtBtMaterial::verify( U32 max_num_passes, float max_detail_level ) 
{
	ITL_TEXTURE_ID tid;
...
GENRESULT DcDtBtMaterial::update( float dt ) 
{
	if( texture_ref_id0 != ITL_INVALID_REF_ID ) {
...
GENRESULT DcDtBtMaterial::apply( void ) 
{
...
GENRESULT DcDtBtMaterial::render( MaterialContext *context, D3DPRIMITIVETYPE PrimitiveType, VertexBufferDesc *Vertices, U32 StartVertex, U32 NumVertices, U16 *FaceIndices, U32 NumFaceIndices, U32 im_rf_flags ) 
{
...
GENRESULT DcDtBtMaterial::set_name( const char *new_name ) 
{
...
GENRESULT DcDtBtMaterial::get_name( char *out_name, U32 max_name_len ) 
{
...
GENRESULT DcDtBtMaterial::get_num_passes( U32 *out_num_passes ) 
{
...
GENRESULT DcDtBtMaterial::get_type( char *out_type, U32 max_type_len )
{
	strncpy( out_type, CLSID_DcDtBtMaterial, max_type_len );
...
GENRESULT DcDtBtMaterial::get_num_textures( U32 *out_num_textures ) 
{
	*out_num_textures = 2;
	return GR_OK;
}
...
GENRESULT DcDtBtMaterial::get_texture( U32 texture_num, ITL_TEXTURE_REF_ID *out_trid ) 
{
	if( texture_num == 0 ) {
...
GENRESULT	DcDtBtMaterial::set_texture(U32 texture_num, ITL_TEXTURE_REF_ID trid) 
{
	ITL_TEXTURE_ID	tid;
...
GENRESULT	DcDtBtMaterial::set_texture_addr_mode(U32 texture_num, TC_COORD which_uv, TC_ADDRMODE mode) 
{
	if(texture_num > 1)
	{
		return	GR_GENERIC;
...
GENRESULT	DcDtBtMaterial::get_texture_addr_mode(U32 texture_num, TC_COORD which_uv, TC_ADDRMODE *out_mode) 
{
	if(texture_num > 1)
	{
		return	GR_GENERIC;
...
GENRESULT	DcDtBtMaterial::set_texture_wrap_mode(U32 texture_num, TC_WRAPMODE mode) 
{
	if(texture_num > 1)
	{
		return	GR_GENERIC;
...
GENRESULT	DcDtBtMaterial::get_texture_wrap_mode(U32 texture_num, TC_WRAPMODE *out_mode) 
{
	if(texture_num > 1)
	{
		return	GR_GENERIC;
...
GENRESULT	DcDtBtMaterial::get_num_constants(U32 *out_num_constants) 
{
	*out_num_constants	=1;
...
GENRESULT	DcDtBtMaterial::set_constant(U32 constant_num, U32 num_values, float *values) 
{
	if(constant_num)
	{
		return	GR_GENERIC;
...
GENRESULT	DcDtBtMaterial::get_constant(U32 constant_num, U32 max_num_values, float *out_values) 
{
	if(constant_num)
	{
		return	GR_GENERIC;
...
GENRESULT DcDtBtMaterial::get_constant_length( U32 constant_num, U32 *out_num_values ) 
{
	if( constant_num > 0 ) {
		return GR_GENERIC;
	}
