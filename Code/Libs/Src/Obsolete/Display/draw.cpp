...
// create DirectDraw object etc.
{
	if (DDraw_active)
	{
		return false; // need to call shutdown!
	}
...
//
	result = CoCreateInstance( CLSID_DirectDraw, NULL, CLSCTX_INPROC_SERVER, IID_IDirectDraw, (LPVOID*)&lpDD);

	if (result != DD_OK)
	{
		DebugAlert(NULL,"DISPLAY: DirectDrawCreate() failed: %s\n",DD_message(result));
		return false;
	}

	result = lpDD->Initialize(0);
	
	if (result != DD_OK)
	{
		DebugAlert(NULL,"DISPLAY: DD->Initialize() failed, code %X\n",result);
		return false;
	}
...
// 
	...
	if (result != DD_OK)
	{
		DebugAlert(NULL,"DISPLAY: DD->SetCooperativeLevel() failed: (1) %s\n", DD_message(result));
		return false;
	}
...
bool DrawMgr::set_display_mode (HWND wnd, int w, int h, int bpp)
{
	if (!DDraw_active)
		return false;
...
// Set exclusive mode
	...
	HRESULT result = lpDD->SetCooperativeLevel(hWnd, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN | DDSCL_ALLOWREBOOT);

	if (result != DD_OK)
	{
		DebugAlert(NULL,"DISPLAY: DD->SetCooperativeLevel() failed: (2) %s\n", DD_message(result));
		return false;
	}

DebugPrint("DISPLAY: lpDD = %X (exclusive)\n",lpDD);

// Set up requested fullscreen display mode

//	result = lpDD->SetDisplayMode(w, h, bpp, 0,0);
	result = lpDD->SetDisplayMode(w, h, bpp);

	if (result != DD_OK)
	{
		DebugAlert(NULL,"DISPLAY: DD->SetDisplayMode() failed, code %X\n",result);
		return false;
	}

DebugPrint("DISPLAY: display mode = %d,%d x %d\n",w,h,bpp);

// pci - try removing EXCLUSIVE lock
	result = lpDD->SetCooperativeLevel(hWnd, DDSCL_NORMAL);
	if (result != DD_OK)
	{
		DebugAlert(NULL,"DISPLAY: DD->SetCooperativeLevel() failed: (3) %s\n", DD_message(result));
		return false;
	}
...
bool DrawMgr::restore_display_mode (HWND hWnd)
{
	if (!DDraw_active || !fullscreen)
		return false;
	...
	if (hWnd)
	{
		...
		if (result != DD_OK)
		{
			DebugAlert(NULL,"DISPLAY: DD->SetCooperativeLevel() failed: (4) %s\n", DD_message(result));
			return false;
		}
		...
		if (result != DD_OK)
		{
			DebugAlert(NULL,"DISPLAY: DD->RestoreDisplayMode() failed: %s\n", DD_message(result));
			return false;
		}
		...
		if (result != DD_OK)
		{
			DebugAlert(NULL,"DISPLAY: DD->SetCooperativeLevel() failed: (5) %s\n", DD_message(result));
			return false;
		}
	}
