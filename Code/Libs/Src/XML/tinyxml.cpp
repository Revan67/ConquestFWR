...
unsigned long TiXmlElement::GetAttributeUnsignedLong( const char* _name )
{
	TiXmlAttribute* node = attributeSet.Find( _name );
	if ( !node )
		return 0;

	unsigned long longValue = node->IntValue();
	return longValue;
}

long TiXmlElement::GetAttributeLong( const char* _name )
{
	TiXmlAttribute* node = attributeSet.Find( _name );
	if ( !node )
		return 0;
...
double TiXmlElement::GetAttributeDouble( const char* _name )
{
	TiXmlAttribute* node = attributeSet.Find( _name );
	if ( !node )
		return 0;
...
float TiXmlElement::GetAttributeFloat( const char* _name )
{
	TiXmlAttribute* node = attributeSet.Find( _name );
	if ( !node )
		return 0;

	float floatValue = (float)node->DoubleValue();
	return floatValue;
}
...
TIXML_QRESULT TiXmlElement::QueryIntAttribute( const char* name, int* ival ) const
{
	TiXmlAttribute* node = attributeSet.Find( name );
	if ( !node )
...
TIXML_QRESULT TiXmlElement::QueryDoubleAttribute( const char* name, double* dval ) const
{
	TiXmlAttribute* node = attributeSet.Find( name );
	if ( !node )
		return TIXML_NO_ATTRIBUTE;
...
TIXML_QRESULT TiXmlElement::QueryUnsignedIntValue( const char* name, unsigned long* uival ) const
{
	TiXmlAttribute* node = attributeSet.Find( name );
	if ( !node )
		return TIXML_NO_ATTRIBUTE;

	return node->QueryUnsignedIntValue( uival );
}

TIXML_QRESULT TiXmlElement::QueryUnicodeValue( const char* name, wchar_t** stringVal ) const
{
	TiXmlAttribute* node = attributeSet.Find( name );
	if ( !node )
		return TIXML_NO_ATTRIBUTE;
	...
	char hexString[5] = {0,0,0,0,0};
	int count = 0;

	static wchar_t* static_ReturnValue = NULL;
	if( static_ReturnValue != NULL )
	{
		delete [] static_ReturnValue;
	}
	static_ReturnValue = new wchar_t[ (valueLen / 4) + 1];

	while( (count*4) < valueLen )
	{
		memcpy( hexString, value, 4 );
		static_ReturnValue[count] = hexstring_to_uword( hexString );
		count++;
		value += 4;
	}
	static_ReturnValue[count] = 0;

	*stringVal = static_ReturnValue;
...
//unsigned int hexstring_to_uword (char *str);
	...
	char* longBuffer = new char[1024*16]; // 16KB long unicode string
	char* pBuffer = longBuffer;

	const wchar_t* walker = value;
	while( walker && *walker )
	{
		sprintf(pBuffer,"%04x", *walker );
		walker++;
		pBuffer += 4;
	}
	sprintf(pBuffer,"0000");

	SetAttribute( name, longBuffer );
	delete longBuffer;
}
...
TIXML_QRESULT TiXmlAttribute::QueryIntValue( int* ival ) const
{
...
TIXML_QRESULT TiXmlAttribute::QueryDoubleValue( double* dval ) const
{
...
TIXML_QRESULT TiXmlAttribute::QueryUnsignedIntValue( unsigned long* uival ) const
{
	if ( sscanf( value.c_str(), "%u", uival ) == 1 )
	{
		return TIXML_SUCCESS;
	}
	return TIXML_WRONG_TYPE;
