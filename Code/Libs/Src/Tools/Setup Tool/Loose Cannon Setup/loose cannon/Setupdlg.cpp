...
_declspec(dllexport) EBURETCODE WINAPI MasterCallback(void *cbd)
{
	...
	{
	case SC_INSTALLLIST:
		return EBU_OK; // CopyFileFn((LPFILECOPYSTATUS) cbd);
...
//		return MkRootFn((LPMKROOTDATA) cbd);

	case SC_GETGROUP:
		return GetGrpFn((LPGETGROUPDATA) cbd);
//		return GetGroupFn((LPGETGROUPDATA) cbd);
   ...
   case SC_INSTDX:
			...
			case QUERYDPLAYINSTALL:
				{
					...
					return InstallDPlay();
					//return QueryDirectPlay(&dxd->uExistingVersion, dxd->gUpdates);
				}break;
				
			case QUERYDIRECTXINSTALL:
				{
					//return EBU_OK;
					return InstallDirectX(dxd->gUpdates);
				}break;
	...
	case SC_DELETEFILE:
		return EBU_OK; // DeleteFileFn(((LPDELETEFILEDATA) cbd)->pszPathName);
...
BOOL DXSetupDlg::Init(LPARAM lParam)
{
	...
	// Add the columns.
	EBULoadString(GetResourceInst(), IDS_DXListColWhatHappensHeader, szText, sizeof(szText));
	ListView_InsertColumn(hWndList, 0, &col);

	EBULoadString(GetResourceInst(), IDS_DXListColRecommendationHeader, szText, sizeof(szText));
	ListView_InsertColumn(hWndList, 1, &col);

	EBULoadString(GetResourceInst(), IDS_DXListColDriverHeader, szText, sizeof(szText));
...
//			MYASSERT(!"ListView_InsertItem failed.");
		...
		nWhatHappenID = IDS_WhatHappensInstall;
		...
		if (FALSE == fAutomatic)
		{
			nRecommendResID = (fRecommend) ? (IDS_RECOMMENDED) : (IDS_NOTRECOMMENDED);
		}else{
			nRecommendResID = IDS_AUTOINSTALL;
		}
					...
					if (tvhti.flags & LVHT_ONITEM)
					{
						if (!Checklist_IsAutomatic(hWndList, iItemClicked)) // not in automatic
						{
							if (Checklist_IsChecked(hWndList, iItemClicked))
							{
								...
								// update happens text
								EBULoadString(GetResourceInst(), IDS_WhatHappensSkip, szMessage, sizeof(szMessage));
								...
								// update happens text
								EBULoadString(GetResourceInst(), IDS_WhatHappensInstall, szMessage, sizeof(szMessage));
								ListView_SetItemText( hWndList, iItemClicked, 0, szMessage);
							}
						}
...
{
	// just determines whether a newer version needs to be installed
	if(*pver == EV_EXISTING_SAME || *pver == EV_EXISTING_NEWER)
	{
		bInstDX = FALSE;
		return EBU_CANCEL;
	}
...
//	   }
//	   return retc;
	   // just start the installation normally - don't change any flags
	   return EBU_OK;
	}
	return EBU_CANCEL;
}
