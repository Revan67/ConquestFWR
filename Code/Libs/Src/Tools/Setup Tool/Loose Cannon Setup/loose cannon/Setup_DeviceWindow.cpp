...
public:		// Interface
	...
	static BOOL EnumAudioDevices( HWND hCombo );
...
//
BOOL CDALibs_DeviceWindow::EnumAudioDevices( HWND hCombo )
{
	if (DirectSoundEnumerate( (LPDSENUMCALLBACK)CDALibs_DeviceWindow::DSoundEnumProc, &hCombo) != DS_OK)
	{
		return( false );
	}
...
{
	EnumAudioDevices(GetDlgItem( hWnd, IDC_AUDIO_COMBO ));
	return S_OK;
}
...
{
	GUID* soundDeviceGUID = NULL;

	if( NotifyCode == CBN_SELCHANGE ) {
		soundDeviceGUID = (GUID *)ComboBox_GetItemData(hControl, ComboBox_GetCurSel(hControl));
	}
	return S_OK;
}
...
void CDALibs_DeviceWindow::bltRenderBuffer(RPLOCKDATA &lock_data, S32 width, S32 height)
{
	...
	HANDLE hBitmap;
	hBitmap = LoadImage(GetResourceInst(), MAKEINTRESOURCE(IDB_HW), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION );

	DIBSECTION dib;
	GetObject (hBitmap, sizeof(DIBSECTION), &dib);

	da_bits = dib.dsBm.bmBits;
	...
	if
	(
		lock_data.pf.get_r_mask() == dib.dsBitfields[0] && 
		lock_data.pf.get_g_mask() == dib.dsBitfields[1] && 
		lock_data.pf.get_b_mask() == dib.dsBitfields[2]
	)
	{
		colorMatch = true;
	}

	if (dib.dsBm.bmWidthBytes == lock_data.pitch)
	{
		pitchMatch = true;
	}
	...
	else if (colorMatch)
	{
		...
		for (int i = 0; i < height; ++i)
		{
			...
			src_row_start += dib.dsBm.bmWidthBytes;
