...
extern bool GetDeviceSelection(HWND hwnd, LPSTR iniFileName, LPSTR szFlags, bool forceConfigureWindow);
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (EBU_ERROR == InitEBUSetup(lpCmd, MasterCallback, NULL, NULL,FALSE,FALSE))
	{
		return FALSE;
	}
	else
	{
		...
		HWND
			hWndPrev = FindWindow(clsname,GetAppTitle());
		if (hWndPrev)
		{
	...
	if(!GetRemovingApp())
	{
		...
		{	// show the device selection dialog box if config was run or "configure" was passed in
			int retVal = CheckDXVersion(REQ_DIRECT_X_VERSION, DIRECT_X_DLL_NAME);
			if ((EV_EXISTING_SAME == retVal) || (EV_EXISTING_NEWER == retVal))
			{
				char inipath[255+1];
				sprintf( inipath, "%s\\%s", GetAppDir(), INI_FILE_NAME);
				ShowWindow(hWnd, SW_HIDE);	/* Shows the window			*/
				if ( GetDeviceSelection(hWnd, inipath, "", forceConfigure) )
...
//		char inipath[255+1];
//		sprintf( inipath, "%s\\%s", GetAppDir(), INI_FILE_NAME);
//		GetDeviceSelection(GetWndParent(), inipath, "");

		if(GetRebootFlag())
		{
			char buf[100];
			EBULoadString(GetResourceInst(),STR_RESTART,buf,100);
			if(MessageBox(GetWndParent(),buf,GetAppTitle(),MB_OKCANCEL | MB_DEFBUTTON2) == IDOK)
			{
				FreeLibrary(GetResourceInst());
...
{	// show the device selection dialog box
	...
	if ((EV_EXISTING_SAME == retVal) || (EV_EXISTING_NEWER == retVal))
	{
		char inipath[255+1];
		sprintf( inipath, "%s\\%s", GetAppDir(), INI_FILE_NAME);
		GetDeviceSelection(GetWndParent(), inipath, "", true);
	}
	else
	{
		char msg[256];
