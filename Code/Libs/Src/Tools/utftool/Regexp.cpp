...
//
// These are the declarations for the C regular expression routines
//
...
static int regexec( const regexp *prog, const char *string );
static void regsub();
...
RegExp::RegExp (char *regex)
{
	set(regex);
}

void RegExp::set( char *regex )
{
	compExp = regcomp (regex);
}
...
bool RegExp::test(const char *string)
{
	start = string;
	b = 0;
	if (compExp) 
	{
		b = regexec ((regexp *) compExp, (char *) string);
	}
	return (b != 0);
}

char *RegExp::get(int i, char *dst, int dstSize, int skip, int retNULL )
{
	register regexp *a = (regexp *) compExp;
	if (!dst)
	{
		dst = buffer;
		dstSize = 80;
	}
	if (a && b && a->startp[i] && a->endp[i])
	{
		int len = a->endp[i]-a->startp[i]-skip;
		len = len>dstSize ? dstSize : len;
		memcpy (dst, a->startp[i]+skip, len);
		dst[len] = 0;
		return dst;
	}
	else
...
int RegExp::getPos(int i)
{
	register regexp *a = (regexp *) compExp;
	if (a && b && a->startp[i])
	{
		return a->startp[i] - a->startp[0];
	}
	return -1;
}

int RegExp::getRealPos(int i)
{
	register regexp *a = (regexp *) compExp;
	if (a && b && a->startp[i])
	{
		return a->startp[i] - start;
	}
	return -1;
}
...
static void reginsert (int, char *);
				...
				if (*regparse == '-') {
					...
					if( *regparse == '\\' ) {
						...
						switch( *regparse++ ) {
							...
							case 'r': regc('\r'); break;
							case '\\': regc('\\'); break;
							case 's': regc(' '); regc('\t'); regc('\n'); regc('\r'); break;
							case 'd': for( int ii='0'; ii<='9'; ii++ ) regc(ii); break;
						}
					}
					else {
						regc(*regparse++);
...
regtry(const regexp *prog, const char *string)
{
	register int i;
	register char **sp;
	register char **ep;
