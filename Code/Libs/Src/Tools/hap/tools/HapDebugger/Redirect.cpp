...
int CRedirect::StdOutThread(HANDLE hStdOutRead)
{
	...
	// Original code would break from this loop if the run thread 
    // flag was cleared. This had the nasty side effetc that the 
    // last little bit of data from the child would not get read
    // from the pipe and posted to the application. Since the thread
    // breaks anyway when the pipe is broken ignoring the flag seems
    // to be pretty safe, and in testing all of the expected data is
    // read from the pipe and sent to the application.
