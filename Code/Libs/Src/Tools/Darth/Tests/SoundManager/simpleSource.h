#ifndef SIMPLE_SOURCE_H
#define SIMPLE_SOURCE_H
...
{
	BEGIN_DACOM_MAP_INBOUND(SoundSource)
	DACOM_INTERFACE_ENTRY(ISoundSource)
	DACOM_INTERFACE_ENTRY2(IID_ISoundSource, ISoundSource)
	END_DACOM_MAP()
...
public:
	...
	virtual GENRESULT COMAPI get_position(Vector *v); 
...
*/
	void set_archetype(SOUND_ARCH_INDEX arch) { m_archetype = arch; };
	void set_start_time(U32 time) { m_startTime = time; };
	void set_on(bool state) { m_on = state; };
	void set_3D(bool state) { m_3D = state; };
