...
void compProjectionIntegrals(P_FACE *f)
{
  ...
  for (i = 0; i < 3; i++) 
  {
	...
	a0 = f->poly->verts[3 * f->verts[i] + A];
    b0 = f->poly->verts[3 * f->verts[i] + B];
    a1 = f->poly->verts[3 * (f->verts[(i+1) % 3]) + A];
    b1 = f->poly->verts[3 * (f->verts[(i+1) % 3]) + B];
    da = a1 - a0;
    db = b1 - b0;
...
#define Z 2
  ...
  p->T0 = p->T1[X] = p->T1[Y] = p->T1[Z] 
		= p->T2[X] = p->T2[Y] = p->T2[Z] 
		= p->TP[X] = p->TP[Y] = p->TP[Z] = 0;
  ...
  for (i = 0; i < p->numFaces; i++) 
  {
    ...
    p->T0 += f->norm[X] * ((A == X) ? Fa : ((B == X) ? Fb : Fc));

    p->T1[A] += f->norm[A] * Faa;
    p->T1[B] += f->norm[B] * Fbb;
    p->T1[C] += f->norm[C] * Fcc;
    p->T2[A] += f->norm[A] * Faaa;
    p->T2[B] += f->norm[B] * Fbbb;
    p->T2[C] += f->norm[C] * Fccc;
    p->TP[A] += f->norm[A] * Faab;
    p->TP[B] += f->norm[B] * Fbbc;
    p->TP[C] += f->norm[C] * Fcca;
  }

  p->T1[X] /= 2; p->T1[Y] /= 2; p->T1[Z] /= 2;
  p->T2[X] /= 3; p->T2[Y] /= 3; p->T2[Z] /= 3;
  p->TP[X] /= 2; p->TP[Y] /= 2; p->TP[Z] /= 2;


  p->mass = density * p->volume;

  /* compute center of mass */
  p->c_o_m[X] = p->T1[X] / p->T0;
  p->c_o_m[Y] = p->T1[Y] / p->T0;
  p->c_o_m[Z] = p->T1[Z] / p->T0;

  /* compute inertia tensor */
  p->i_t[X][X] = density * (p->T2[Y] + p->T2[Z]);
  p->i_t[Y][Y] = density * (p->T2[Z] + p->T2[X]);
  p->i_t[Z][Z] = density * (p->T2[X] + p->T2[Y]);
  p->i_t[X][Y] = p->i_t[Y][X] = - density * p->TP[X];
  p->i_t[Y][Z] = p->i_t[Z][Y] = - density * p->TP[Y];
  p->i_t[Z][X] = p->i_t[X][Z] = - density * p->TP[Z];

  /* translate inertia tensor to center of mass */
  p->i_t[X][X] -= p->mass * (p->c_o_m[Y]*p->c_o_m[Y] + p->c_o_m[Z]*p->c_o_m[Z]);
  p->i_t[Y][Y] -= p->mass * (p->c_o_m[Z]*p->c_o_m[Z] + p->c_o_m[X]*p->c_o_m[X]);
  p->i_t[Z][Z] -= p->mass * (p->c_o_m[X]*p->c_o_m[X] + p->c_o_m[Y]*p->c_o_m[Y]);
  p->i_t[X][Y] = p->i_t[Y][X] += p->mass * p->c_o_m[X] * p->c_o_m[Y]; 
  p->i_t[Y][Z] = p->i_t[Z][Y] += p->mass * p->c_o_m[Y] * p->c_o_m[Z]; 
  p->i_t[Z][X] = p->i_t[X][Z] += p->mass * p->c_o_m[Z] * p->c_o_m[X]; 
