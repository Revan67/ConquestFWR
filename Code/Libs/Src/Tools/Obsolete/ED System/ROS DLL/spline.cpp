...
void calculateCRSplineTangent(Vector *p0, Vector *p1, Vector *p2, Vector *p3, SINGLE t, Vector *tangent)
{
	SINGLE t2 = t * t;
	...
	tangent->x =	0.5 *	(	(3 * t2 * (-p0->x + (p1->x * 3) - (p2->x * 3) + p3->x)) +
								(2 * t * ((p0->x * 2) - (p1->x * 5) + (p2->x * 4) - p3->x)) +
								(-p0->x + p2->x)
							);

	tangent->y =	0.5 *	(	(3 * t2 * (-p0->y + (p1->y * 3) - (p2->y * 3) + p3->y)) +
								(2 * t * ((p0->y * 2) - (p1->y * 5) + (p2->y * 4) - p3->y)) +
								(-p0->y + p2->y)
							);
	
	tangent->z =	0.5 *	(	(3 * t2 * (-p0->z + (p1->z * 3) - (p2->z * 3) + p3->z)) +
								 (2 * t * ((p0->z * 2) - (p1->z * 5) + (p2->z * 4) - p3->z)) +
								 (-p0->z + p2->z)
							);
}
