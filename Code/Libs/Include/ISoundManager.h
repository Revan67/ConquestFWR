...
/*
  ...
  DWORD get_speaker_configuration ();
	...
	GENRESULT COMAPI set_master_reverb(const U32 baseEnv, const SINGLE vol, const SINGLE decay, const SINGLE damping)
		INPUT:
	...
	GENRESULT COMAPI get_master_reverb(U32 *baseEnv, SINGLE *vol, SINGLE *decay, SINGLE *damping)
		INPUT:
			NONE
...
#define IID_ISoundArchetype MAKE_IID("ISoundArchetype", 1)

struct ISoundArchetype : public IDAComponent
{
	virtual void COMAPI get_sound_format(SoundFormat *) = 0;
	virtual U32 COMAPI get_samples(void * samples) = 0;
	virtual U32 COMAPI get_sample_count() = 0;
	virtual SINGLE COMAPI get_base_attenuation() = 0;
	virtual U32 COMAPI get_num_channels() = 0;
	virtual U32 COMAPI get_duration() = 0;

	virtual void COMAPI set_samples(void * samples, U32 length) = 0;
	virtual GENRESULT COMAPI set_base_attenuation(SINGLE attenuation) = 0;
	virtual bool COMAPI is_loopable() = 0;
};

#define ISOUNDMANAGER_VERSION 1
#define IID_ISoundManager MAKE_IID("ISoundManager", 1)
struct ISoundManager : public IDAComponent
{
// global/primary buffer modifiers
	virtual GENRESULT COMAPI set_sound_format(SoundFormat *soundFormat) = 0;
	virtual GENRESULT COMAPI get_sound_format(SoundFormat *soundFormat) = 0;

	virtual GENRESULT COMAPI set_master_attenuation(const SINGLE attenuation) = 0;
	virtual GENRESULT COMAPI set_maximum_channels(const U32 numChannels) = 0;
	virtual GENRESULT COMAPI set_speaker_configuration(const U32 speakerConfiguration) = 0;

	virtual SINGLE COMAPI get_master_attenuation() = 0;
	virtual U32 COMAPI get_maximum_channels() = 0;
	virtual U32 COMAPI get_speaker_configuration() = 0;

	// 3D EAX/A3D/Property set extensions (for A3D only the damping factor is used)
	virtual GENRESULT COMAPI set_master_reverb(const U32 baseEnv, const SINGLE vol, const SINGLE decay, const SINGLE damping) = 0;
	virtual void COMAPI get_master_reverb(U32 *baseEnv, SINGLE *vol, SINGLE *decay, SINGLE *damping) = 0;

	// don't use these methods.  implement ISoundListener and use update(ISoundListener *IEar)
// listener property modifiers	
	virtual GENRESULT COMAPI set_ear_orientation(const Matrix * orientation) = 0;
	virtual GENRESULT COMAPI set_ear_orientation(const Vector *front, const Vector *top) = 0;
	virtual GENRESULT COMAPI set_ear_position(const Vector *) = 0;
	virtual GENRESULT COMAPI set_ear_velocity(const Vector *) = 0;
	virtual GENRESULT COMAPI set_ear_distance_factor(const SINGLE) = 0;
	virtual GENRESULT COMAPI set_ear_doppler_factor(const SINGLE) = 0;
	virtual GENRESULT COMAPI set_ear_rolloff_factor(const SINGLE) = 0;
	virtual void COMAPI get_ear_orientation(Matrix * orientation) = 0;
	virtual void COMAPI get_ear_orientation(Vector *front, Vector *top) = 0;
	virtual Vector COMAPI get_ear_position() = 0;
	virtual Vector COMAPI get_ear_velocity() = 0;
	virtual SINGLE COMAPI get_ear_distance_factor() = 0;
	virtual SINGLE COMAPI get_ear_doppler_factor() = 0;
	virtual SINGLE COMAPI get_ear_rolloff_factor() = 0;

	// Expansion mechanism.
	virtual GENRESULT COMAPI get_property(ISoundSource *sound, REFGUID propGUID, const U32 propID, void *propData, const U32 sizeOfPropData, U32 * sizeOfDataWritten) = 0;
	virtual GENRESULT COMAPI get_global_property(REFGUID propGUID, const U32 propID, void *propData, const U32 sizeOfPropData, U32 * sizeOfDataWritten) = 0;
	virtual GENRESULT COMAPI set_property(ISoundSource *sound, REFGUID propGUID, const U32 propID, void *propData, const U32 sizeOfPropData) = 0;
	virtual GENRESULT COMAPI set_global_property(REFGUID propGUID, const U32 propID, void *propData, const U32 sizeOfPropData) = 0;

	// Archetype management
	virtual SOUND_ARCH_INDEX COMAPI create_archetype_from_raw_data (const SoundFormat &, U32 length, U32 loop_start, U32 loop_end, void *sample_buffer, U32 options) = 0;
	virtual SOUND_ARCH_INDEX COMAPI create_archetype_from_soundfile (const SoundFile &sourceData, U32 options) = 0;
	virtual SOUND_ARCH_INDEX COMAPI create_archetype (IFileSystem *sourceFile, U32 options) = 0;
	virtual GENRESULT COMAPI destroy_archetype (SOUND_ARCH_INDEX archetype) = 0;
	virtual GENRESULT COMAPI get_archetype_interface(SOUND_ARCH_INDEX archetype, void ** archInterface) = 0;

	// Playbuffer management
	virtual GENRESULT COMAPI add_active_sound (ISoundSource *sound, U32 index) = 0;
	virtual GENRESULT COMAPI remove_active_sound (ISoundSource *sound) = 0;
	virtual GENRESULT COMAPI set_active_sounds (ISoundSource *sounds[], U32 count) = 0;
	virtual U32 COMAPI get_active_sound_count () = 0;
	virtual U32 COMAPI get_playing_sound_count () = 0;

	// Misc.
	virtual GENRESULT COMAPI startup(HWND hWnd, U32 options) = 0;
	virtual GENRESULT COMAPI shutdown() = 0;
	virtual GENRESULT COMAPI get_directsound_interface(void **) = 0;
	virtual U32 get_current_time_ms() = 0;
	virtual GENRESULT get_device_info(SM_DEVICEINFO *) = 0;
	virtual void COMAPI update (ISoundListener *IEar = NULL) = 0;
};
...
#define IID_ISoundManagerStatus MAKE_IID("ISoundManagerStatus", 1)
struct ISoundManagerStatus : public IDAComponent
{
	...
	virtual GENRESULT COMAPI get_playing_sound_status (int which, SoundStatus &playingStatus) = 0;
};
