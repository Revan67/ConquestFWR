...
struct CQPipeline : IRenderPipeline
{
	...
	virtual ID3DXEffect* load_effect(char * filename,IComponentFactory *DIR)
	{
		...
		{
			EnterCriticalSection(criticalSection);
			effect = pipe->load_effect( filename, DIR );
			LeaveCriticalSection(criticalSection);
		}
		else
		{
	...
	virtual GENRESULT COMAPI set_vs_constants(UINT StartRegister,CONST FLOAT* pConstantData,UINT RegisterCount)
	{
		PIPETHROUGH ( set_vs_constants(StartRegister, pConstantData, RegisterCount) )
	}

	virtual GENRESULT COMAPI set_ps_constants(UINT StartRegister,CONST FLOAT* pConstantData,UINT RegisterCount)
	{
		PIPETHROUGH ( set_ps_constants(StartRegister, pConstantData, RegisterCount) )
	}

	virtual GENRESULT COMAPI set_vertex_declaration(IDirect3DVertexDeclaration9 *pDecl)
	{
		PIPETHROUGH ( set_vertex_declaration(pDecl) )
	}

	virtual GENRESULT COMAPI set_default_constants( const Transform &W, Vector eye)
	{
		PIPETHROUGH ( set_default_constants(W, eye) )
	}
