...
void TreeRender(MeshChain &mc,bool bLight, DWORD color)
{
	...
	if (mc.mi[0]->bHasMesh && bLight)
	{
		...
		LIGHTS->ActivateBestLights(pos,8,max(mc.mi[0]->radius*2,500));
	}
...
void TreeRender(MeshInfo **mi,int num_children)
{
	if (mi[0]->bHasMesh)
	{
		...
		LIGHTS->ActivateBestLights(pos,8,max(mi[0]->radius*2,500));
	}
	bScaleTrans = false;
...
void TreeRender(MeshInfo *mi,const Transform &_scaleTrans)
{
	if (mi->bHasMesh)
	{
		...
		LIGHTS->ActivateBestLights(pos,8,max(mi->radius*2,500));
	}
	bScaleTrans = true;
...
void MeshRender::SetupMeshInfo(MeshInfo *mc,bool bMakeBuffers, const char* materialOverride)
{
	if (!testLightCurves)
		{
			testEffect = PIPE->load_effect("cq2\\shaders\\test.fx",(IComponentFactory *)TEXTURESDIR);
			testLightCurves = TMANAGER->CreateTextureFromFile("cq2\\LightCurves.tga", (IComponentFactory *)TEXTURESDIR, DA::TGA, PF_4CC_DAA8);		
		}
...
void FaceGroupRender::Init(Mesh *mesh,int _fg_idx,MeshRender *_mr)//MeshInfo *mc)
{
	...
	if (mesh->material_list[fg->material].emissive_texture_id != 0)
		flags |= MM_MULTITEX;
	...
	for (i=0;i<fg->face_cnt;i++)
	{
		...
		bool bSecondSet = GET_TC_WRAP_MODE(mesh->material_list[fg->material].emissive_texture_flags) == TC_WRAP_UV_1;
...
//	memset(re_index_list,0xff,sizeof(U16)*new_face_cnt*3);
	...
	//	CQASSERT(v_max < vert_cnt);
		...
		if (fgi[fg_idx].colorTex && !fgi[fg_idx].bCQ2Mat)
		{
		...
		else if (fgi[fg_idx].colorTex)
		{
			...
			count++;
			UINT passes;
			testEffect->Begin(&passes, 0);
			testEffect->Pass(0);
			//GENRESULT result = PIPE->draw_indexed_primitive_vb( D3DPT_TRIANGLELIST, vb_handle, vb_offset, v_max+1, id_list, index_count, 0 );

			//GENRESULT result = PIPE->draw_indexed_primitive_vb( D3DPT_TRIANGLELIST, vb_handle, vb_offset, v_max+1, index_list4[0],new_face_cnt*3 , 0 );
			GENRESULT result = PIPE->draw_indexed_primitive_vb( D3DPT_TRIANGLELIST, vb_handle, vb_offset, maxVert[0], IB4[0],new_face_cnt*3 , 0 );
			CQASSERT(result == GR_OK);
			testEffect->End();
				...
				if (cnt1)
				{
					...
					for (i=0;i<cnt1;i++)
					{
						...
						if (re_index_list[v_src] == 0xffff)
						{
							...
							dest_verts[v_dst].v2 = src_verts[v_src].v;
							//dest_verts[v_dst] = src_verts[v_src];
