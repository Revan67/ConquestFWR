...
#define D3DFVF_VBVERTEX (D3DFVF_XYZ | D3DFVF_NORMAL |D3DFVF_DIFFUSE| D3DFVF_TEX5)
...
//	S32 index_count;
	...
	PIPE->set_default_constants(world_adjust_tr,cam_pos_in_object_space,4);
...
void MeshRender::Init(ARCHETYPE_INDEX idx)
{
	...
	if (mesh)
	{
		...
		for (int a=0;a<2;a++)
		{
			...
			{
				mesh->imaterial_list = new IMaterial*[mesh->material_cnt];
				for (int material_entry = 0; material_entry < mesh->material_cnt; material_entry++)
				{
					char texName[255];
					...
					if (!texName[0])
					{
						// toDo: cleanup properly and prevent leaks
						//delete mesh->imaterial_list;
						mesh->imaterial_list = 0;
						break;
					}
					else 
					{
						...
						sprintf(fileName, "%s.txt", texName);
						if (!strstr(fileName, "color."))
						{
							char* tmp = strstr(fileName, ".");
							sprintf(tmp, "%s","_color.tga.txt");
						}
						mesh->imaterial_list[material_entry] = MATMAN->FindMaterial(fileName);

						if(!mesh->imaterial_list[material_entry])  
						{
							strlwr(fileName);
							mesh->imaterial_list[material_entry] = MATMAN->FindMaterial(fileName);
						}
						if(mesh->imaterial_list[material_entry])  
						{
							...
							if (tmp) 
							{
								...
								mesh->imaterial_list[material_entry]->SetTextureName("Diffuse + Opacity + Height",texName);
								sprintf(tmp, "_bump.tga");
								mesh->imaterial_list[material_entry]->SetTextureName("Normal + Specularity",texName);
							}
							mesh->imaterial_list[material_entry]->Realize();
						}
						else
						{
							// toDo: cleanup properly and prevent leaks
				...
				{
					// toDo: cleanup properly and prevent leaks
					mesh->imaterial_cnt = mesh->material_cnt;
					mesh->material_list = 0;
					mesh->material_cnt = 0;
				}
...
void MeshRender::BuildTangentSpace(VBVertex * pVertices, int c)
{
	...
	for( i=0; i < pos_cnt; i++ )
    {
		pVertices[i].r = rand()%255;
		pVertices[i].g = rand()%255;
		pVertices[i].b = rand()%255;
		pVertices[i].a = rand()%255;
	...
	//	CQASSERT(v_max < vert_cnt);
		...
		else if (fgi[fg_idx].colorTex)
		{
			...
			if (bDebugShader && count%300 == 0) 
			{
				//emissiveEffect = PIPE->load_effect("cq2\\shaders\\emissive.fx",(IComponentFactory *)TEXTURESDIR);
				//nonEmissiveEffect = PIPE->load_effect("cq2\\shaders\\noEmissive.fx",(IComponentFactory *)TEXTURESDIR);
				emissiveEffect = nonEmissiveEffect = PIPE->load_effect("cq2\\shaders\\noEmissive.fx",(IComponentFactory *)TEXTURESDIR);
			...
			*/
			...
			if (passes == 1)
			{
				...
				chosenEffect->BeginPass(0);
				GENRESULT result = PIPE->draw_indexed_primitive_vb( D3DPT_TRIANGLELIST, vb_handle, vb_offset, maxVert[0], IB4[0],new_face_cnt*3 , 0 );
				chosenEffect->EndPass();
				CQASSERT(result == GR_OK);
			}
